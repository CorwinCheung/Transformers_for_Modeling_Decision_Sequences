...
from utils.file_management import (get_experiment_file, get_latest_run,
                                   parse_model_info, convert_to_local_path)
...
def plot_bpos_behavior_learning(predictions, model_name, run):
    ...
    bpos = pts.calc_bpos_probs(predictions, add_cond_cols=['Step'], add_agg_cols=['pred_Switch', 'pred_selHigh'])
    fig, axs = pts.plot_bpos_behavior(bpos, hue='Step', palette='viridis', alpha=0.3,
                                    plot_features={
...
def add_choice_metrics(df, prefix=''):
    ...
    df['prev_choice'] = df['Context'].apply(lambda x: x[-1].upper())
...
def main(run=None, model_name=None):
    ...
    for N in [2, 3]:
        predictions[f'seq{N}_RL'] = predictions['Context'].apply(lambda x: x[-N:])
        predictions[f'seq{N}'] = predictions[f'seq{N}_RL'].apply(map_rl_to_pattern)
        policies_true = pts.calc_conditional_probs(predictions.query('Step==Step.max()'), htrials=N, sortby='pevent', pred_col='Switch')
        fig, ax = pts.plot_sequences(policies_true)

        policies_pred = pts.calc_conditional_probs(predictions, htrials=N, sortby='pevent', pred_col='pred_Switch', add_grps='Step')
        fig, ax = pts.plot_sequence_points(policies_pred, grp='Step', palette='viridis', yval='pevent', size=3, ax=ax, fig=fig)
        if N > 2:
